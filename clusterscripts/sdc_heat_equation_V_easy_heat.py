...
def solve_heat_pde(
    ...
    *,
    ...
    degree: int = 3,  # p=3 como en tu comando
    analysis: bool = True,  # queremos métrica de colocación
    mode: str = "checkpoint",
    folder_name: str | None = None,
    path_name: str | None = None,
):
    # ---- Nombre del archivo (detallado) ----
    dt_str = f"{dt:.2e}".replace(".", "p")
    Tfinal_str = f"{Tfinal:.2e}".replace(".", "p")
    residual = "par" if is_parallel else "global"
    file_name = (
        f"heatHOMO_n{n_cells}_dt{dt_str}_sw{nsweeps}_nodes{M}_deg{degree}_"
        f"prectype{prectype}_{residual}_tfinal{Tfinal_str}"
    )

    Tfinal_eff = dt if (Tfinal is None or float(Tfinal) <= 0.0) else float(Tfinal)

    base_dir = _resolve_base_output_dir(path_name)
    base_dir.mkdir(parents=True, exist_ok=True)

    # si caller da path_name absoluto, no anidar subcarpetas
    ...
    # Solución exacta (calor homogéneo): u_t - Δu = 0, u = sin(pi x) e^{-pi^2 t}
    def u_exact(Xcoord, t):
        return sin(pi * Xcoord[0]) * exp(-(pi**2) * t)

    # Forzamiento nulo (¡clave del test homogéneo!)
    f_obtained = lambda t: Constant(0.0)

    # Forma débil para SDC: u_t = Δu + f  => f_i(t,u,v) = -<∇u,∇v> + <f,v>
    def f_heat(t, u, v):
        return -inner(grad(u), grad(v)) + f_obtained(t) * v

    # CI y CC compatibles con la exacta
    ...
    pde = PDESystem(
        ...
        name="Heat1D_Homogeneous",
    )

    solver = SDCSolver(
        mesh=mesh,
        PDEs=pde,
        M=M,  # Radau-right (por defecto) ⇒ orden 2M-1 = 3
        dt=dt,
        is_parallel=is_parallel,
        solver_parameters={  # lineal ⇒ LU va bien; tolerancias estrictas si quieres
            "snes_type": "newtonls",
            "snes_rtol": 1e-12,
            "snes_atol": 1e-14,
            "ksp_type": "preonly",
        ...
        prectype=prectype,  # MIN-SR-FLEX como en tu comando
        file_name=file_name,
        folder_name=folder_name,
        path_name=path_name_str,
        analysis=analysis,  # << necesario para tener solución de colocación y métricas
        mode=mode,
    )

    real_exp = u_exact if analysis else None
    solver.solve(Tfinal_eff, nsweeps, real_exp)


# --- main: compatible con tu PBS/qsub (usa SDC_PARAMS_JSON) ---
if __name__ == "__main__":
    ...
    if cfg:
        p = json.loads(cfg)
        # OBLIGATORIO: M = sweeps (tus lanzadores ya lo hacen)
        ...
        solve_heat_pde(
            ...
            analysis=bool(p.get("analysis", True)),  # << debe ser True
            mode=str(p.get("mode", "checkpoint")),
            folder_name=p.get("folder_name"),
            path_name=p.get("path_name"),
        )
    else:
        # Default local rápido (por si corres sin PBS)
        solve_heat_pde(
            dt=1e-2,
            n_cells=512,
            nsweeps=2,
